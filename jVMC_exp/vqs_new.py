...
import jVMC_exp.global_defs_new as global_defs
...
class NQS:
    ...
    def init_net(self, seed: int | None):
        ...
        dtype = global_defs.DT_SAMPLER
        param_dtype = self.param_dtype
        name = 'DT_SAMPLER'

        if dtype is not None:
            is_dtype_complex = jnp.issubdtype(dtype, jnp.complexfloating)
            if is_dtype_complex and self.realParams:
                raise ValueError(
                    f'The dtype of the parameters (real={self.realParams}) of the model '
                    f'and of {name} (real={self.realParams}) are not compatible.'
                )
