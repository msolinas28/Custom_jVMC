...
comm = MPI.COMM_WORLD
rank = comm.Get_rank()
commSize = comm.Get_size() # Number of Processes 
...
def distribute_sampling_sh(numSamples, numChains, mesh) -> tuple[int, int, int]:
    """
    Distribute sampling tasks across sharded devices.

    For a desired total number of samples, determines how many samples
    should be generated by each Monte Carlo chain.

    Arguments:
        numSamples: Total number of samples desired.
        numChains: Total number of chains across all devices.
        mesh: JAX Mesh object.

    Returns:
        (totalChains, chainsPerDevice, samplesPerChain): 
            - totalChains: Actual number of chains used (adjusted if needed)
            - chainsPerDevice: Number of chains per device
            - samplesPerChain: Number of samples each chain generates
    """
    total_devices = mesh.shape["devices"]
    
    # Ensure numChains >= total_devices and divisible by total_devices
    if numChains < total_devices:
        print(f"WARNING: Number of chains ({numChains}) is smaller than total devices ({total_devices}).")
        print(f"         Increased to: {total_devices}")
        numChains = total_devices
    elif numChains % total_devices != 0:
        # Round up to next multiple of total_devices
        adjusted_chains = ((numChains + total_devices - 1) // total_devices) * total_devices
        print(f"WARNING: Number of chains ({numChains}) not divisible by devices ({total_devices}).")
        print(f"         Increased to: {adjusted_chains}")
        numChains = adjusted_chains
    
    chainsPerDevice = numChains // total_devices
    
    # Round UP to ensure we get at least numSamples total
    samplesPerChain = (numSamples + numChains - 1) // numChains
    totalSamples = samplesPerChain * numChains
    samplesPerDevice = chainsPerDevice * samplesPerChain
    
    if totalSamples > numSamples:
        print(f"INFO: Total samples adjusted: {numSamples} -> {totalSamples}")
    
    return numChains, chainsPerDevice, totalSamples, samplesPerDevice
